...
$$\text{total grad re} = \sum_{i=0}^{N-1} \left( A_{re}[i] \cdot r_{re}[i] + A_{im}[i] \cdot r_{im}[i] \right)$$

$$\text{total grad im} = \sum_{i=0}^{N-1} \left( A_{re}[i] \cdot r_{im}[i] - A_{im}[i] \cdot r_{re}[i] \right)$$

The parameter update vector state rules for optimization follow:

$$x_{re}^{(k+1)} = x_{re}^{(k)} + \left( \alpha \cdot \text{total grad re} \right)$$

$$x_{im}^{(k+1)} = x_{im}^{(k)} + \left( \alpha \cdot \text{total grad im} \right)$$
---
